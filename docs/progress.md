...
1. **로드맵 3.2 시작**: 한국어 도메인 이슈 조사 (숫자, 단위, 띄어쓰기, 고유명사, 영어/약어 등)
2. **라벨 스키마 정의**: sample_id 기준 필드 정의 (`text_raw`, `text_verified`, `text_normalized`, 트리아지 메타데이터)
3. **최소 파이프라인 실행 계획 수립**: 트리아지 기준 초안, 검수 운영 방식 설계
